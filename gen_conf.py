...
def should_print_empty_line(key, value, already_printed):
    if len(value) > 1 and key not in ['option', 'server'
...
def update_haproxy_conf_with_domains(haproxy_conf, domains_conf):
    ...
    for i, kv in enumerate(domains_conf.items()):
        key, value = kv
        domains_string = ' '.join(
            [f'-i {domain}' for domain in value['domains']])
        ...
        inbound_template['frontend inbound']['use_backend'].append(
            f'domain{i} if is_domain{i}')
        server_lines = []
        check_string = ''
